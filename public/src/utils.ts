import { startOfMonth, subMonths } from "date-fns";
import { formatInTimeZone, fromZonedTime, toZonedTime } from "date-fns-tz";
import { ja } from "date-fns/locale";

/* ==========================================================================
   Constants
   ========================================================================== */

/**
 * 日本時間のタイムゾーン識別子。
 */
const TIMEZONE = "Asia/Tokyo";

/**
 * 金額マスク表示用のラベル。
 */
export const MASKED_LABEL = "¥ *****";

/**
 * システムによる残高調整用カテゴリID。
 */
export const SYSTEM_BALANCE_ADJUSTMENT_CATEGORY_ID =
	"SYSTEM_BALANCE_ADJUSTMENT";

/**
 * アプリのテーマカラー定義。
 * Chart.jsなどのJS側で描画するUIコンポーネントの色を一元管理する。
 */
export const THEME_COLORS = {
	primary: "#4f46e5",
	primaryLight: "#eef2ff",
	primaryRing: "rgba(79, 70, 229, 0.2)",
	success: "#16a34a",
	successLight: "#dcfce7",
	danger: "#dc2626",
	dangerLight: "#fee2e2",
	neutral: {
		text: "#374151",
		subtext: "#6b7280",
		border: "#e5e7eb",
		grid: "#e5e7eb",
		bg: "#f9fafb",
	},
};

/* ==========================================================================
   Internal State
   ========================================================================== */

// スクロール制御用
let scrollPosition = 0;
let scrollLockCount = 0;

// 通貨フォーマッター（再利用のためキャッシュ）
const currencyFormatter = new Intl.NumberFormat("ja-JP", {
	style: "currency",
	currency: "JPY",
});

const compactCurrencyFormatter = new Intl.NumberFormat("ja-JP", {
	notation: "compact",
});

/* ==========================================================================
   Date Utilities
   ========================================================================== */

// 日付フォーマットの計算結果キャッシュ
const yyyymmCache = new Map<string | number, string>();
const yyyymmddCache = new Map<string | number, string>();
const weekdayCache = new Map<string | number, string>();

function getDateCacheKey(date: Date | string | number): string | number {
	if (date instanceof Date) return date.getTime();
	return String(date);
}

/**
 * Dateオブジェクトを日本時間基準の 'yyyy-MM' 形式の文字列に変換する。
 * @param date - 変換対象の日付。
 * @returns 'yyyy-MM' 形式の文字列。
 */
export function toYYYYMM(date: Date | string | number): string {
	const key = getDateCacheKey(date);
	let res = yyyymmCache.get(key);
	if (!res) {
		const d = date instanceof Date ? date : new Date(date);
		res = formatInTimeZone(d, TIMEZONE, "yyyy-MM");
		if (yyyymmCache.size > 500) yyyymmCache.clear();
		yyyymmCache.set(key, res);
	}
	return res;
}

/**
 * Dateオブジェクトを日本時間基準の 'yyyy-MM-dd' 形式の文字列に変換する。
 * @param date - 変換対象の日付。
 * @returns 'yyyy-MM-dd' 形式の文字列。
 */
export function toYYYYMMDD(date: Date | string | number): string {
	const key = getDateCacheKey(date);
	let res = yyyymmddCache.get(key);
	if (!res) {
		const d = date instanceof Date ? date : new Date(date);
		res = formatInTimeZone(d, TIMEZONE, "yyyy-MM-dd");
		if (yyyymmddCache.size > 500) yyyymmddCache.clear();
		yyyymmddCache.set(key, res);
	}
	return res;
}

/**
 * Dateオブジェクトを 'yyyy年M月d日(曜日)' 形式の文字列に変換する。
 * 日本時間基準でフォーマットする。
 * @param date - 変換対象の日付。
 * @returns 'yyyy年M月d日(曜日)' 形式の文字列。
 */
export function formatDateWithWeekday(date: Date | string | number): string {
	const key = getDateCacheKey(date);
	let res = weekdayCache.get(key);
	if (!res) {
		const d = date instanceof Date ? date : new Date(date);
		res = formatInTimeZone(d, TIMEZONE, "yyyy年M月d日(EEE)", {
			locale: ja,
		});
		if (weekdayCache.size > 500) weekdayCache.clear();
		weekdayCache.set(key, res);
	}
	return res;
}

/**
 * 現在の日付をローカルタイム基準の 'yyyy-MM-dd' 形式の文字列で取得する。
 * input[type="date"] の初期値などに使用する。
 * @returns 'yyyy-MM-dd' 形式の現在日付文字列。
 */
export function getLocalToday(): string {
	const now = new Date();
	const year = now.getFullYear();
	const month = String(now.getMonth() + 1).padStart(2, "0");
	const day = String(now.getDate()).padStart(2, "0");
	return `${year}-${month}-${day}`;
}

/**
 * 指定された月数前の月の開始日時（日本時間）をUTCに変換して取得する。
 * @param months - 戻る月数。
 * @returns UTCのDateオブジェクト。
 */
export function getStartOfMonthAgo(months: number): Date {
	const nowInTokyo = toZonedTime(new Date(), TIMEZONE);
	const startDate = startOfMonth(subMonths(nowInTokyo, months));
	return fromZonedTime(startDate, TIMEZONE);
}

/**
 * 指定された年の開始日時（日本時間）をUTCに変換して取得する。
 * @param year - 対象年。
 * @returns UTCのDateオブジェクト。
 */
export function getStartOfYear(year: number): Date {
	const startDate = new Date(year, 0, 1);
	return fromZonedTime(startDate, TIMEZONE);
}

/**
 * 指定された年の終了日時（日本時間）をUTCに変換して取得する。
 * @param year - 対象年。
 * @returns UTCのDateオブジェクト。
 */
export function getEndOfYear(year: number): Date {
	const endDate = new Date(year, 11, 31, 23, 59, 59);
	return fromZonedTime(endDate, TIMEZONE);
}

/**
 * 日付オブジェクトを日本時間として解釈し、UTCのDateオブジェクト（Timestamp保存用）に変換する。
 * @param date - 変換対象の日付。
 * @returns UTCのDateオブジェクト。
 */
export function toUtcDate(date: Date): Date {
	return fromZonedTime(date, TIMEZONE);
}

/* ==========================================================================
   Formatting Utilities
   ========================================================================== */

/**
 * 数値を日本円形式にフォーマットする。
 * @param amount - 金額。
 * @param isMasked - マスク表示するかどうか。
 * @returns 例: "¥1,000" または "¥ *****"
 */
export function formatCurrency(amount: number, isMasked = false): string {
	if (isMasked) return MASKED_LABEL;
	return currencyFormatter.format(amount).replace(/[￥\\]/g, "¥");
}

/**
 * 数値をグラフ等のために短縮フォーマットする（例: 1万）。
 * @param value - 数値。
 * @param isMasked - マスク表示するかどうか。
 * @returns 例: "1.5万"
 */
export function formatLargeCurrency(value: number, isMasked = false): string {
	if (isMasked) return "¥***";
	if (value === 0) return "0";
	return compactCurrencyFormatter.format(value).replace(/[￥\\]/g, "¥");
}

/**
 * 文字列から一意のHEXカラーコードを生成する。
 * @param str - 入力文字列。
 * @returns HEXカラーコード。
 */
export function stringToColor(str: string): string {
	let hash = 0;
	for (let i = 0; i < str.length; i++) {
		hash = str.charCodeAt(i) + ((hash << 5) - hash);
	}
	let color = "#";
	for (let i = 0; i < 3; i++) {
		const value = (hash >> (i * 8)) & 0xff;
		color += ("00" + value.toString(16)).slice(-2);
	}
	return color;
}

/* ==========================================================================
   DOM Utilities
   ========================================================================== */

/**
 * iOS Safari等でのスクロールロックを制御する。
 * モーダル表示時などに背景がスクロールしないように固定する。
 * @param isLocked - ロックするかどうか。
 */
export function toggleBodyScrollLock(isLocked: boolean): void {
	const body = document.body;
	if (isLocked) {
		if (scrollLockCount === 0) {
			scrollPosition = window.scrollY;
			body.style.position = "fixed";
			body.style.top = `-${scrollPosition}px`;
			body.style.width = "100%";
			body.classList.add("modal-open");
		}
		scrollLockCount++;
	} else {
		scrollLockCount = Math.max(0, scrollLockCount - 1);
		if (scrollLockCount === 0) {
			body.style.position = "";
			body.style.top = "";
			body.style.width = "";
			body.classList.remove("modal-open");
			window.scrollTo({ top: scrollPosition, behavior: "instant" });
		}
	}
}

/* ==========================================================================
   Data & Logic Utilities
   ========================================================================== */

export interface SortableItem {
	id?: string;
	type?: string;
	order?: number;
	name?: string;
}

/**
 * アイテム配列をソートする（種類 > 順序 > 名前）。
 * @param items - ソート対象のアイテム配列。
 * @returns ソート済み配列。
 */
export function sortItems<T extends SortableItem>(items: T[]): T[] {
	return [...items].sort((a, b) => {
		// 1. 種類 (asset優先)
		if (a.type !== b.type) {
			if (a.type === "asset") return -1;
			if (b.type === "asset") return 1;
		}
		// 2. 順序
		const orderA = a.order ?? Infinity;
		const orderB = b.order ?? Infinity;
		if (orderA !== orderB) {
			return orderA - orderB;
		}
		// 3. 名前
		return (a.name || "").localeCompare(b.name || "");
	});
}

/**
 * 入力文字列から数値以外を除去する。
 * @param value - 入力文字列。
 * @returns 数値のみの文字列。
 */
export function sanitizeNumberInput(value: string): string {
	let sanitized = value.replace(/[^0-9.]/g, "");
	const parts = sanitized.split(".");
	if (parts.length > 2) {
		sanitized = parts[0] + "." + parts.slice(1).join("");
	}
	return sanitized;
}

export interface SummaryTransaction {
	type: "income" | "expense" | "transfer" | string;
	amount: number;
	categoryId: string;
}

export interface CategorySummaryItem {
	id: string;
	amount: number;
	name: string;
	color: string;
}

export interface TransactionSummary {
	income: number;
	expense: number;
	balance: number;
	incomeDetails: CategorySummaryItem[];
	expenseDetails: CategorySummaryItem[];
}

/**
 * 取引データから収支サマリーを計算する。
 * @param transactions - 取引データの配列。
 * @param luts - ルックアップテーブル。
 * @returns サマリーオブジェクト。
 */
export function summarizeTransactions(
	transactions: SummaryTransaction[],
	luts: {
		categories?: Map<string, { id?: string; name: string }>;
	},
): TransactionSummary {
	let incomeTotal = 0;
	let expenseTotal = 0;
	const incomeCats: Record<string, number> = {};
	const expenseCats: Record<string, number> = {};

	transactions.forEach((t) => {
		if (t.type === "income") {
			incomeTotal += t.amount;
			incomeCats[t.categoryId] = (incomeCats[t.categoryId] || 0) + t.amount;
		} else if (t.type === "expense") {
			expenseTotal += t.amount;
			expenseCats[t.categoryId] = (expenseCats[t.categoryId] || 0) + t.amount;
		}
	});

	const processCats = (catsObj: Record<string, number>): CategorySummaryItem[] => {
		if (!luts.categories) return [];
		return Object.entries(catsObj)
			.map(([id, amount]) => {
				const cat = luts.categories?.get(id);
				return {
					id,
					amount,
					name: cat
						? cat.name
						: id === SYSTEM_BALANCE_ADJUSTMENT_CATEGORY_ID
							? "残高調整"
							: "未分類",
					color: cat ? stringToColor(cat.name) : "#9CA3AF",
				};
			})
			.sort((a, b) => b.amount - a.amount);
	};

	return {
		income: incomeTotal,
		expense: expenseTotal,
		balance: incomeTotal - expenseTotal,
		incomeDetails: processCats(incomeCats),
		expenseDetails: processCats(expenseCats),
	};
}
