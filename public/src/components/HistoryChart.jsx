/**
 * 資産推移チャットを表示するReactコンポーネント。
 * Chart.jsを使用して、月ごとの純資産、収入、支出を可視化する。
 ...
 * @param {boolean} props.isMasked - 金額をマスク表示するかどうか。
 * @returns {JSX.Element} チャートを含むコンテナ要素。
 */
export default function HistoryChart({ historicalData, isMasked }) {
    ...
    useEffect(() => {
        if (!isChartReady || !canvasRef.current || !historicalData || historicalData.length === 0) return;
    ...
    return (
        <div style={{ width: '100%', height: '100%', position: 'relative', minWidth: '600px', minHeight: '350px' }}>
