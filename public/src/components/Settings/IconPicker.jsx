...
export default function IconPicker({ isOpen, onClose, onSelect }) {
    useEffect(() => {
        ...
        window.addEventListener('keydown', handleKeyDown, true); // true for capturing phase to ensure we intercept before parent
    ...
    return (
        <div className="fixed inset-0 modal-overlay z-60 flex justify-center items-center p-4 bg-black/50" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
            <div className="bg-white rounded-lg shadow-xl w-full max-w-lg">
                ...
                <div className="p-4 grid grid-cols-6 sm:grid-cols-8 md:grid-cols-10 gap-2 max-h-[60vh] overflow-y-auto">
                 ...
                 <div className="p-4 border-t border-neutral-200 flex justify-end">
                    <button onClick={onClose} className="px-4 py-2 bg-neutral-100 rounded-lg hover:bg-neutral-200 text-sm font-bold text-neutral-700">キャンセル</button>
                 </div>
            </div>
        </div>
    );
