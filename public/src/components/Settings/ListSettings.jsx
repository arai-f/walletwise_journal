...
const SYSTEM_BALANCE_ADJUSTMENT_CATEGORY_ID = "system_balance_adjustment"; // Hardcoded or import? utils.SYSTEM_BALANCE_ADJUSTMENT_CATEGORY_ID

export default function ListSettings({ type, title, store, getState, refreshApp }) {
    ...
    // Add CSS for Sortable visual feedback
    useEffect(() => {
    ...
    const loadItems = () => {
        ...
        if (type === 'asset' || type === 'liability') {
            fetchedItems = [...luts.accounts.values()].filter(a => a.type === type && !a.isDeleted);
        } else {
            fetchedItems = [...luts.categories.values()].filter(c => c.type === type && !c.isDeleted);
        }
    ...
    const handleSort = async () => {
        ...
        const orderedIds = [...listRef.current.children].map(child => child.dataset.id);
        try {
            if (type === 'asset' || type === 'liability') {
    ...
    const handleAddItem = async () => {
        ...
        try {
            let currentCount = (type === "asset" || type === "liability") 
                ? luts.accounts.size 
    ...
    return (
        <div className="p-4">
             <div className="flex justify-between items-center mb-4">
                 <h3 className="font-bold text-neutral-800 border-l-4 border-primary pl-3">{title}</h3>
                 <button 
             ...
             {isAdding && (
                 <div className="mb-4 flex items-center gap-2 p-2 rounded-md bg-neutral-100 animate-fade-in">
                     ...
                     />
                     <button onClick={handleAddItem} className="text-success hover:text-success-dark p-1"><i className="fas fa-check"></i></button>
                     <button onClick={() => setIsAdding(false)} className="text-danger hover:text-danger-dark p-1"><i className="fas fa-times"></i></button>
                 </div>
             )}
             ...
             {/* Balance Adjustment Section for Assets */}
             {type === 'asset' && (
                <div className="mt-8 pt-6 border-t border-neutral-200">
                    <h3 className="font-bold text-neutral-800 mb-4 border-l-4 border-primary pl-3">残高調整</h3>
...
function ListItem({ item, type, store, getState, refreshApp, reloadList, balances, onEditIcon }) {
    ...
    const itemType = (type === 'asset' || type === 'liability') ? 'account' : 'category';
    ...
    const handleDelete = async () => {
        if(type === 'asset' || type === 'liability') {
             if (!confirm(`口座「${item.name}」を本当に削除しますか？\n（取引履歴は消えません）`)) return;
             await store.deleteItem(item.id, 'account');
        } else {
            const targetName = (type === 'income') ? PROTECTED_DEFAULTS[0] : PROTECTED_DEFAULTS[1];
            if (!confirm(`カテゴリ「${item.name}」を削除しますか？\nこのカテゴリの既存の取引はすべて「${targetName}」に振り替えられます。`)) return;
            
             const { luts } = getState();
             const toCategory = [...luts.categories.values()].find(c => c.name === targetName);
             if(!toCategory) return alert(`振替先のカテゴリ「${targetName}」が見つかりません。`);
    ...
    return (
        <div className="flex items-center justify-between p-3 rounded-md bg-white shadow-sm transition hover:shadow-md mb-2 group" data-id={item.id}>
                     ...
                     {isEditing ? (
                         <div className="flex items-center gap-1">
                             <input 
                                ...
                                autoFocus
                             />
                             <button onClick={handleSave} className="text-success p-1"><i className="fas fa-check"></i></button>
                         </div>
                     ) : (
                         <span className="block truncate font-medium text-neutral-900 text-base">{item.name}</span>
                     )}
                 </div>
             </div>
             ...
             <div className="flex items-center gap-1 shrink-0">
                 {isEditable && !isEditing && (
                    <button onClick={() => setIsEditing(true)} className="text-primary hover:text-primary-dark p-2 rounded-lg hover:bg-neutral-100 transition" title="名前を編集">
                        <i className="fas fa-pen"></i>
                    </button>
                 )}
                 {isDeletable ? (
                     <button onClick={handleDelete} className="text-danger hover:text-danger-dark p-2 rounded-lg hover:bg-neutral-100 transition" title="削除">
                         <i className="fas fa-trash-alt"></i>
                     </button>
                 ): (
                     <div className="p-2 text-neutral-400 cursor-help" title={tooltip}><i className="fas fa-lock"></i></div>
                 )}
             </div>
        </div>
    );
...
function BalanceAdjustItem({ account, currentBalance, store, refreshApp, utils }) {
    ...
    const handleAdjust = async () => {
        ...
        if (confirm(`「${account.name}」の残高を ¥${difference.toLocaleString()} 調整しますか？`)) {
            const transaction = {
                type: difference > 0 ? "income" : "expense",
                date: utils.toYYYYMMDD(new Date()),
                amount: Math.abs(difference),
                categoryId: utils.SYSTEM_BALANCE_ADJUSTMENT_CATEGORY_ID || "system_balance_adjustment", 
    ...
    return (
        <div className="flex flex-col md:grid md:grid-cols-5 md:items-center gap-2 md:gap-4 p-3 rounded-md bg-neutral-50">
            <span className="font-medium text-neutral-900 md:col-span-2">{account.name}</span>
