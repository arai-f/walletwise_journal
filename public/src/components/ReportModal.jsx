...
const ReportModal = ({ isOpen, onClose, luts }) => {
    ...
    const stats = useMemo(() => {
        ...
        yearData.forEach(t => {
            const amount = Number(t.amount);
             if (t.type === 'income') {
                 income += amount;
                 incomeMap.set(t.categoryId, (incomeMap.get(t.categoryId) || 0) + amount);
             } else if (t.type === 'expense') {
                 expense += amount;
                 expenseMap.set(t.categoryId, (expenseMap.get(t.categoryId) || 0) + amount);
             }
        });
        ...
        const formatCategoryStats = (map, total) => {
             return Array.from(map.entries()).map(([id, val]) => {
                 ...
                 return {
                     ...
                     textColor: cat?.type === 'income' ? 'text-emerald-600' : 'text-rose-600'
                 };
    ...
    const handleExport = () => {
         ...
         const rows = yearData.map(t => {
             ...
             const typeLabel = t.type === 'income' ? '収入' : t.type === 'expense' ? '支出' : '振替';
                     ...
                     <div className="flex items-center gap-4">
                         ...
                         <div className="flex p-1 bg-neutral-200 rounded-lg">
                             <button
                                 ...
                                 className={`px-3 py-1 text-xs font-bold rounded-md transition ${activeTab === 'monthly' ? 'bg-white text-neutral-800 shadow-sm' : 'text-neutral-500 hover:text-neutral-700'}`}
                             >
                                 月別推移
                             </button>
                             <button
                                 onClick={() => setActiveTab('category')}
                                 className={`px-3 py-1 text-xs font-bold rounded-md transition ${activeTab === 'category' ? 'bg-white text-neutral-800 shadow-sm' : 'text-neutral-500 hover:text-neutral-700'}`}
                             >
                                 カテゴリ別
                             </button>
                         </div>
                         ...
                         <div className="space-y-6">
                             ...
                             <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                 <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100">
                                     <div className="text-emerald-600 text-xs font-bold mb-1">収入</div>
                                     <div className="text-2xl font-bold text-neutral-800">¥{stats.income.toLocaleString()}</div>
                                 </div>
                                 <div className="bg-rose-50 p-4 rounded-xl border border-rose-100">
                                      <div className="text-rose-600 text-xs font-bold mb-1">支出</div>
                                      <div className="text-2xl font-bold text-neutral-800">¥{stats.expense.toLocaleString()}</div>
                                 </div>
                                 <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100">
                                      <div className="text-indigo-600 text-xs font-bold mb-1">収支</div>
                                      <div className={`text-2xl font-bold ${stats.balance >= 0 ? 'text-neutral-800' : 'text-red-500'}`}>
                             ...
                             {yearData.length > 0 ? (
                                             ...
                                             <table className="w-full text-sm text-left">
                                                 ...
                                                 <tbody className="divide-y divide-neutral-100">
                                                     {Array.from({length: 12}, (_, i) => i + 1).map(m => {
                                                         const monthlyTxns = yearData.filter(t => new Date(t.date).getMonth() + 1 === m);
                                                         
                                                         const mInc = monthlyTxns.filter(t => t.type === 'income').reduce((sum, t) => sum + Number(t.amount), 0);
                                                         const mExp = monthlyTxns.filter(t => t.type === 'expense').reduce((sum, t) => sum + Number(t.amount), 0);
                                                         ...
                                                         return (
                                                             <tr key={m}>
                                                                 ...
                                                                 <td className="p-3 text-emerald-600">+{mInc.toLocaleString()}</td>
                                                                 <td className="p-3 text-rose-600">-{mExp.toLocaleString()}</td>
                                                                 <td className="p-3 font-bold">{ (mInc - mExp).toLocaleString() }</td>
                                                             </tr>
                                                         );
                                                     })}
                                                 </tbody>
                                         ...
                                         <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            ...
                                            <div className="space-y-4">
                                                <h3 className="font-bold text-neutral-700 flex items-center">
                                                    <i className="fas fa-minus-circle text-rose-500 mr-2"></i>支出内訳
                                                </h3>
                                                    ...
                                                    {stats.categoryBreakdown.expense.map(cat => (
                                                        <div key={cat.id}>
                                                            <div className="flex justify-between text-sm mb-1">
                                                                ...
                                                                <span className="font-bold text-neutral-900">¥{cat.amount.toLocaleString()} ({Math.round(cat.percentage)}%)</span>
                                                            </div>
                                            ...
                                            <div className="space-y-4">
                                                <h3 className="font-bold text-neutral-700 flex items-center">
                                                    <i className="fas fa-plus-circle text-emerald-500 mr-2"></i>収入内訳
                                                </h3>
                                                <div className="space-y-3">
                                 ...
                                 <p className="text-neutral-400 text-center">データがありません</p>
                             )}
                         </div>
                     )}
                 </div>
