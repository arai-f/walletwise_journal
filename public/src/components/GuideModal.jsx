    ...
    useEffect(() => {
        if (isOpen && !htmlContent && !isLoading) {
            ...
            fetch('guide.html?t=' + Date.now())
                ...
                .catch(err => {
                    console.error(err);
                    setHtmlContent('<div class="p-4 text-red-500">ガイドの読み込みに失敗しました</div>');
                    setIsLoading(false);
                });
        }
    }, [isOpen, htmlContent, isLoading]);

    // Initialize Swiper & Dynamic Content
    useEffect(() => {
        if (htmlContent && containerRef.current && isOpen) {
            ...
            setTimeout(async () => {
                 ...
                 // 2. Dynamic Summary (setupAnalysisSummaryGuide)
                 const summaryContainer = containerRef.current.querySelector("#analysis-math-summary-guide");
                 if (summaryContainer) {
                 ...
                 // 3. Notification Button (setupNotificationButtonListener)
                 const btn = containerRef.current.querySelector("#guide-enable-notification");
                 if (btn) {
                     // Check current state
