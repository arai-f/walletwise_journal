...
export function openModal(transaction = null, prefillData = null) {
	...
	if (data.type === "transfer") {
		...
		document.getElementById("category").value =
			data.category || config.expenseCategories[0];
		document.getElementById("payment-method").value =
			data["payment-method"] || data.paymentMethod || config.assets[0];
	}
}
...
function setupFormForType(type) {
	...
	if (type === "transfer") {
		...
		fromSelect.value = config.assets[0];
		toSelect.value = config.assets[1] || allAccounts[1];
	} else {
		const categorySelect = document.getElementById("category");
		const paymentMethodSelect = document.getElementById("payment-method");
		populateSelect(
			document.getElementById("category"),
			type === "income" ? config.incomeCategories : config.expenseCategories
		);
		populateSelect(document.getElementById("payment-method"), allAccounts);
		categorySelect.selectedIndex = 0;
		paymentMethodSelect.value = config.assets[0];
	}
}
